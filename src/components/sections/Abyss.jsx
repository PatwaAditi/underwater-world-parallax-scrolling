...
import Submarine from '../creatures/Submarine';
import Shipwreck from '../creatures/Shipwreck';

const Abyss = ({ onCreatureClick }) => {
  return (
    <section id="abyss" className="relative h-screen w-full overflow-hidden bg-gradient-to-b from-purple-900 via-slate-900 to-primary">
      {/* Background Video */}
      <video
        autoPlay
        loop
        ...
        onError={(e) => {
          ...
          if (video) {
            ...
            fallback.className = 'absolute top-0 left-0 w-full h-full bg-gradient-to-b from-purple-900 via-slate-900 to-primary z-0';
      ...
      <div className="relative z-20 flex flex-col items-center justify-center h-full text-center px-4 pointer-events-none">
        <h2 className="text-4xl md:text-6xl font-bold text-white mb-4">
          <span className="text-glow">The Abyss</span>
        </h2>
        <p className="text-md md:text-lg text-gray-400 max-w-2xl">
          Pressure and darkness guard the secrets of the deep. Wrecks of the past meet the explorers of the future.
        </p>
      </div>

      {/* Objects */}
      <div className="absolute top-0 left-0 w-full h-full z-30">
        <Submarine onClick={() => onCreatureClick(creatureData.submarine)} />
        <Shipwreck onClick={() => onCreatureClick(creatureData.shipwreck)} />
      </div>
